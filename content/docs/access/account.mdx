...
- Include mandatory acknowledgements in related publications.
- Correctly attach e-INFRA CZ as a large research infrastructure source in R&D results.

<Callout type="info">
The acknowledgements and proper reporting procedures allow the ongoing provision of advanced computing resources to the academic and research institutions in the Czech Republic.
</Callout>

### Mandatory Acknowledgment Text in Publications

Your publications must include the following acknowledgement:
Czech version
```
...
5. Enter your home organization's credentials (your home login and password).
...
Non-web services typically authenticate via login/password, Kerberos, or SSH keys. Please take a look at the respective service documentation for precise instructions.


## e-INFRA CZ Login/Password Management

By default, your e-INFRA CZ Account doesn't require new credentials; however, some services necessitate specific e-INFRA CZ username/password credentials. You'll first need to create these after using such a service. Once defined, these credentials are uniform across all services requiring them and managed at the [e-INFRA CZ User Profile](https://profile.e-infra.cz).
